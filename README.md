...
- [`docs/セキュリティ指針.md`](docs/セキュリティ指針.md) — **このプロジェクトに依存しない規則。**
  「他人のアプリの認証情報を読み、内部 API に相乗りする」ツールを作る人なら
  誰にでも当てはまる形にしてある。**全部、実際に踏んだことから抽出したもの**
- [`docs/調査-JSONL集計.md`](docs/調査-JSONL集計.md) — **作らないと決めた機能**の調査記録。
  実装が存在しない機能の設計を本編に混ぜないための隔離で、気が変わったときの出発点
